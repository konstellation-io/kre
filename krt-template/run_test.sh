...
RUNTIME=$1
VERSION=$2
DATA_FILE=${3:-"./test/greeter.json"}

ENTRYPOINT_SERVICE=entrypoint.Entrypoint/Greet
...
call_to_entrypoint() {
    ...
    ## GRPC CALL
    RESPONSE=$(cat "$DATA_FILE" | grpcurl -plaintext -d @ "$ENTRYPOINT_URL" "$ENTRYPOINT_SERVICE")
