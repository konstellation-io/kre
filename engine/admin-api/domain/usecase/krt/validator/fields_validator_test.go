...
func TestYamlFieldsValidator_Run(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			krtYaml:     NewKrtBuilder().V2().WithVersion("this-version-name-length-is-higher-than-the-maximum").Build(),
			wantError:   true,
		...
		},
		{
			...
			krtYaml:     NewKrtBuilder().V2().WithDescription("").Build(),
			wantError:   true,
			errorString: "the field \"Description\" is required",
		},
		{
			name:        "fails if proto doesn't end with .proto",
			krtYaml:     NewKrtBuilder().V2().WithEntrypointProto("invalid").Build(),
			wantError:   true,
