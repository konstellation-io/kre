...
func TestStoreDashboard(t *testing.T) {
	...
	s.mocks.dashboardService.EXPECT().Create(context.Background(), version, dashboardPath).Return(nil)

	err := s.versionInteractor.storeDashboards(context.Background(), dashboardsFolder, version)
	assert.Nil(t, err)
}
