  ...
  import {isEntrypoint, NodeWithPosition} from "../nodes/nodeUtils";
  ...
  function drawNodeCountBadge(ctx: CanvasRenderingContext2D, node: NodeWithPosition, x: number, y: number, statusColor: string, exitpoint: string) {
    ...
    console.log(styles.colorStatusStopped)
    ...
    ctx.beginPath();
    ctx.font = 'bolder 4pt Calibri';
    ctx.fillStyle = "black";
    ctx.textAlign = "center";
    ctx.fillText(node.replicas.toString(), x + 2 * Math.PI, y - 2 * Math.PI + 2);
  }
