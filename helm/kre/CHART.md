...
| entrypoints.ingress.annotations | object | See `entrypoint.grpc.ingress.annotations` in [values.yaml](./values.yaml)  | GRPC Ingress annotations |
| entrypoints.ingress.tls.secretName | string | If not deffined, every created ingress will use an autogenerated certificate name based on the chart relase fullname. Otherwise, the specified secret must exists and contain a valid TLS certificate. | TLS certificate secret name. A wildcard for the current application domain must be used. |
| entrypoints.tls | bool | `false` | Wether to enable tls |
...
| k8sManager.krtFilesDownloader.image.pullPolicy | string | `"Always"` | Image pull policy for `krt-files-downloader` |
| k8sManager.krtFilesDownloader.image.repository | string | `"konstellation/krt-files-downloader"` | Image repository for `krt-files-downloader` |
| k8sManager.krtFilesDownloader.image.tag | string | `"latest"` | Image tag for `krt-files-downloader` |
