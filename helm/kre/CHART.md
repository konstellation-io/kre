...
| k8sManager.generatedEntrypoints.ingress.tls.secretName | string | If not defined, every created ingress will use an autogenerated certificate name based on the chart relase fullname. Otherwise, the specified secret must exists and contain a valid TLS certificate. | TLS certificate secret name. A wildcard for the current application domain must be used. |
