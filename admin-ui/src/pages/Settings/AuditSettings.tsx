  ...
  return (
    <div className={styles.formField}>
      ...
      <div className={styles.button}>
        <Button label={'SEARCH'} onClick={onSubmit} border />
      </div>
    </div>
  );
}
...
function isInputInvalid(value: string) {
  return CHECK.getValidationError([CHECK.isFieldAnString(value)]);
}

function AuditSettings() {
  const [usersFilter, setUsersFilter] = useState('');
  const { value, onChange, error } = useInput('', isInputInvalid);
  ...
  return (
    <>
      <div className={cx(styles.form, styles.securitySettings)}>
        ...
        />
        <FormField error={error} onChange={onChange} onSubmit={onApplySearch} />
